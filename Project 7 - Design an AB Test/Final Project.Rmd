...
- It is very likely that net conversion will stay the same. If the net conversion will decrease that would be negative feedback for our experiment. 
...
- net conversion will stay the same as the number of students continuing the cours after 14 days trial has not changed,
- retention would increase as only motivated students potentially joined the trial (rest was eliminated by the screener).
...
I have decided not to use Bonferroni correction. In general the Bonferroni correction is used to add correction to the experiment whre multiple metrics are tracked but only in the case when some of them need to be significant in order to launch the change. Our experiment requires both metrics (gross conversion and net conversion) to be partically significant in order to decide to implement changes on the website. I believe that adding this correction would be  too conservative and secrease false positive risk. Moreover, I think that it is better to make decision on introducing the change on the website taking into the consideration significance of rather two than one metric.  
